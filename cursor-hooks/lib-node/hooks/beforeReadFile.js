...
class BeforeReadFileHook extends PermissionHook {
  ...
  async makeDecision() {
    ...
    for (const pattern of BeforeReadFileHook.SENSITIVE_PATTERNS) {
      ...
      if (re.test(lower)) {
        try {
          await this.sendToOrtensia(`Agent 要读取敏感文件：${fileName}，需要确认`, "surprised");
        } catch {}
    ...
    if (["config", "setting"].some((k) => lower.includes(k))) {
      try {
        await this.sendToOrtensia(`Agent 正在读取配置：${fileName}`, "neutral");
      } catch {}
    }
...
module.exports = (async () => {
  const code = await new BeforeReadFileHook().run();
  return code;
})();
