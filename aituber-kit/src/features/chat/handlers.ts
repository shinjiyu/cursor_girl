...
export const handleReceiveTextFromWsFn =
  ...
  async (
    ...
    if (ss.externalLinkageMode) {
      console.log('✅ [handleReceiveTextFromWs] ExternalLinkage Mode: true')
    } else {
    ...
    if (role !== 'user') {
      ...
      if (role === 'assistant' && text !== '') {
        console.log('🟣 [handleReceiveTextFromWs] Calling speakCharacter with:', { message: text, emotion: emotion, audio_file: audio_file })
        try {
          // 文ごとに音声を生成 & 再生、返答を表示
          speakCharacter(
            sessionId,
            {
              message: text,
              emotion: emotion,
              audio_file: audio_file,  // 传递 TTS 音频文件路径
            },
            () => {
              console.log('🎬 [speakCharacter] onStart callback')
              // assistantMessage is now derived from chatLog, no need to set it separately
            },
            () => {
              console.log('🎬 [speakCharacter] onComplete callback')
              // hs.decrementChatProcessingCount()
            }
          )
        } catch (e) {
          console.error('🔴 [speakCharacter] Error:', e)
        }
      } else {
        console.log('⚠️ [handleReceiveTextFromWs] Skipped speakCharacter:', { role, textEmpty: text === '' })
      }
