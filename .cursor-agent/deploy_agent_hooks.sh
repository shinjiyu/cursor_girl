#!/bin/bash
# Agent Hooks 部署脚本
# 将 Agent Hooks 部署到 ~/.cursor-agent/
...
if [ -f "$CURSOR_DIR/hooks.json" ]; then
    ...
    mv "$CURSOR_DIR/hooks.json" "$CURSOR_DIR/hooks.json.backup"
fi

echo -e "${GREEN}🔗 创建配置符号链接...${NC}"
ln -s "$TARGET_DIR/hooks.json" "$CURSOR_DIR/hooks.json"
...
ls -lh "$TARGET_DIR/hooks/" | grep "\.py$" | awk '{print "   • " $9 " (" $5 ")"}'

# 统计 hooks 数量
HOOK_COUNT=$(ls -1 "$TARGET_DIR/hooks/"*.py | wc -l | tr -d ' ')
echo ""
